...
#    df = pd.DataFrame({'chrom1': col_chr, 'start1': col_pos})
    df = pd.DataFrame({'chrom1': col_chr,
                       'start1': col_left, 
                       'end1': col_right,
                       'size': col_sizes,
                       'heights': heights,
...
#
    ...
    aax1.plot(coverage_plasmid_new, color="royalblue")
    ...
    for i in range(len(pos1)):
        rectangle = plt.Rectangle( (min(pos1[i],pos2[i])/BIN, 0.1),
