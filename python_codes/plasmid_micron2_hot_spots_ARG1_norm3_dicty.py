...
genes = pd.read_csv('/media/axel/RSG4/dicty/genes_proteins_56_22720.txt', 
                    sep="\t", header= None, skiprows=0)
long_genes = genes[ abs(genes[3]-genes[2]) > 10000 ]
len(long_genes)
...
for chr1 in list_chr1 :
    ...
    if BIN == 200:
        ...
        hot_spots = find_peaks(coverage_plasmid_new,
                               height = 0.0011,
                               width = 5.,
                               distance=70)  # for 200 bp resolutionn
...
#    coverage_plasmid = coverage_plasmid /(float(sizes_dist[chr2]))
    ...
    limit_y =   np.nanmax(coverage_plasmid)
...
#    plt.xlim(0, limit_x)
    ...
    plt.plot(hot_spots[0],hot_spots[0]/hot_spots[0]*median_plasmid*8.,'v', color="orange")
    h=hot_spots[1]
    left_ips = h['left_ips']
...
#    plt.plot(right_ips,right_ips*0.0,'|')
    ...
    ax4.plot(coverage_plasmid_new, color="royalblue")
    ax4.fill_between(range(len(coverage_plasmid_new)),coverage_plasmid_new, facecolor='royalblue')
